...
import numpy as np
from .Onedrive.Python_Blender.blend_auto_rig.Custom_Auto_Rig import ClassTest
# from ClassTest import Test
# O:\Onedrive\Python Blender\blend_auto_rig\Custom Auto Rig
# dir = 'O:\Onedrive\Python Blender\blend_auto_rig\Custom Auto Rig\ClassTest.py'
...
create_root_bone(rig) #4
parent_bone(rig, 'pelvis', 'ROOT', False)
...
for item in toe_rotation_to_duplicate:
    if item[-1] == 'L':
        ...
        l_pos_1, l_pos_2 = bone_global_locations(rig, item)
        the_roll = bone_roll(rig, item)
        new_bone_creation(l_pos_1, l_pos_2, new_name, rig, 'toes_L', False, the_roll)
        renamer(rig, new_name, new_name.replace('_', '', 1))
    else:
        new_name = item[:-3] + 'Rotation_R'
        r_pos_1, r_pos_2 = bone_global_locations(rig, item)
        the_roll = bone_roll(rig, item)
        new_bone_creation(r_pos_1, r_pos_2, new_name, rig, 'toes_L', False, the_roll)
        renamer(rig, new_name, new_name.replace('_', '', 1))

close_toes_L = ['bigToe_1_L', 'indexToe_1_L', 'ringToe_1_L', 'pinkyToe_1_L', 'midToe_1_L']        
for item in close_toes_L:
    lock_rotation(rig, item, y=True)
    lock_rotation(rig, (item[:-4] + 'Rotation_L'), y=True, z=True)
    copy_rotation(rig, item, subtarget=(item[:-3] + 'Rotation_L'),use_x=True, target_space='LOCAL', owner_space='LOCAL' )
close_toes_R =  ['bigToe_1_R', 'indexToe_1_R', 'ringToe_1_R', 'pinkyToe_1_R', 'midToe_1_R']
for item in close_toes_R:
    lock_rotation(rig, item, y=True)
    lock_rotation(rig, (item[:-4] + 'Rotation_R'), y=True, z=True)
    copy_rotation(rig, item, subtarget=(item[:-3] + 'Rotation_R'),use_x=True, target_space='LOCAL', owner_space='LOCAL')
...
for item in left_rest_toes:
    ...
    copy_rotation(rig, item, mix_mode='AFTER', subtarget=(item[:-3] + 'Rotation_L'), use_x=True, target_space='LOCAL', owner_space='LOCAL')

for item in right_rest_toes:
    lock_rotation(rig, item, y=True, z=True)
    copy_rotation(rig, item, mix_mode='AFTER', subtarget=(item[:-3] + 'Rotation_R'), use_x=True, target_space='LOCAL', owner_space='LOCAL')
...
remove_edit_and_arm_selection(rig)
duplicate_bones(rig, 'footFK_L', 'MCH_foot_L', False, False, bone_roll(rig, 'footFK_L'), keep_selection=False)
duplicate_bones(rig, 'toes_L', 'toesIK_L', False, False, bone_roll(rig, 'toes_L'), keep_selection=False)
duplicate_bones(rig, 'footFK_R', 'MCH_foot_R', False, False, bone_roll(rig, 'footFK_R'), keep_selection=False)
duplicate_bones(rig, 'toes_R', 'toesIK_R', False, False, bone_roll(rig, 'toes_L'), keep_selection=False)


# 3-4
left_foot_tip_empty_location, left_foot_tip_empty_world = object_location('footTip_Empty_L')
left_foot_end_empty_location, left_foot_end_empty_world = object_location('footEnd_Empty_L')
left_Knee_empty_location, left_Knee_empty_world = object_location('knee_Empty_L')
right_foot_tip_empty_location, right_foot_tip_empty_world = object_location('footTip_Empty_R')
right_foot_end_empty_location, right_foot_end_empty_world = object_location('footEnd_Empty_R')
right_Knee_empty_location, right_Knee_empty_world = object_location('knee_Empty_R')

new_bone_creation(left_foot_tip_empty_location, left_foot_end_empty_location, 'footIK_L', rig, 'ROOT', False, 1.5708)
new_bone_creation(right_foot_tip_empty_location, right_foot_end_empty_location, 'footIK_R', rig, 'ROOT', False, 1.5708)


# 5 - 6
extrude_out('MCH_foot_L', 'footRollCTRL_L', rig, True, False, (0, 0.1, 0), 0.038292, 0)
parent_bone(rig, 'footRollCTRL_L', 'footIK_L', False)
extrude_out('MCH_foot_R', 'footRollCTRL_R', rig, True, False, (0, 0.1, 0), 0.038292, 0)
parent_bone(rig, 'footRollCTRL_R', 'footIK_R', False)

# 7 - 8
pos_1, pos_2 = bone_global_locations(rig, 'MCH_foot_L')
new_bone_creation(pos_2, left_foot_end_empty_location, 'MCH_footRoll_1_L', rig, 'ROOT', False, 0)
position_0 = left_foot_end_empty_location[0]
position_1 = left_foot_end_empty_location[1]
move_head_or_tail('MCH_footRoll_1_L', rig, False, True, 0, (position_0, position_1, pos_2[2]))

pos_1, pos_2 = bone_global_locations(rig, 'MCH_foot_R')
new_bone_creation(pos_2, right_foot_end_empty_location, 'MCH_footRoll_1_R', rig, 'ROOT', False, 0)
position_0 = right_foot_end_empty_location[0]
position_1 = right_foot_end_empty_location[1]
move_head_or_tail('MCH_footRoll_1_R', rig, False, True, 0, (position_0, position_1, pos_2[2]))

left_change = bone_length('MCH_footRoll_1_L', rig)
right_change = bone_length('MCH_footRoll_1_R', rig)
change_bone_length('MCH_footRoll_1_L', rig, (left_change / 4))
change_bone_length('MCH_footRoll_1_R', rig, (right_change / 4))

# 9-10
parent_bone(rig, 'MCH_foot_L', 'MCH_footRoll_1_L', False)
parent_bone(rig, 'MCH_foot_R', 'MCH_footRoll_1_R', False)
parent_bone(rig, 'MCH_footRoll_1_L', 'footIK_L', False)
parent_bone(rig, 'MCH_footRoll_1_R', 'footIK_R', False)

# 11-13
eular_change(rig, 'footRollCTRL_R', 'XYZ')
eular_change(rig, 'footRollCTRL_L', 'XYZ')
lock_location(rig, 'footRollCTRL_R', x=True, y=True, z=True)
lock_location(rig, 'footRollCTRL_L', x=True, y=True, z=True)
lock_scale(rig, 'footRollCTRL_R', x=True, y=True, z=True)
lock_scale(rig, 'footRollCTRL_L', x=True, y=True, z=True)
lock_rotation(rig, 'footRollCTRL_R', z=True)
lock_rotation(rig, 'footRollCTRL_L', z=True)

# 14-15
copy_rotation(rig, 'MCH_footRoll_1_L',subtarget='footRollCTRL_L', use_x=True, target_space='LOCAL', owner_space='LOCAL')
copy_rotation(rig, 'MCH_footRoll_1_R',subtarget='footRollCTRL_R', use_x=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_1_L', max_x=degree_to_radians(170), limit_x=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_1_R', max_x=degree_to_radians(170), limit_x=True, target_space='LOCAL', owner_space='LOCAL')

# 16-21
start_point, global_pos, end_point, global_alt = object_location('footEnd_Empty_L', y=0.042891, alter=True)
new_bone_creation(start_point, end_point, 'heelPosition_L', rig, 'ROOT', False, 0)
start_point, global_pos, end_point, global_alt = object_location('footEnd_Empty_R', y=0.042891, alter=True)
new_bone_creation(start_point, end_point, 'heelPosition_R', rig, 'ROOT', False, 0)

copy_rotation(rig, 'heelPosition_L',subtarget='footRollCTRL_L', use_x=True, target_space='LOCAL', owner_space='LOCAL')
copy_rotation(rig, 'heelPosition_R',subtarget='footRollCTRL_R', use_x=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'heelPosition_L', min_x=degree_to_radians(-170), limit_x=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'heelPosition_R', min_x=degree_to_radians(-170), limit_x=True, target_space='LOCAL', owner_space='LOCAL')

parent_bone(rig, 'MCH_footRoll_1_L', 'heelPosition_L', False)
parent_bone(rig, 'MCH_footRoll_1_R', 'heelPosition_R', False)
parent_bone(rig, 'toesIK_L', 'heelPosition_L', False)
parent_bone(rig, 'toesIK_R', 'heelPosition_R', False)

# 22-27
pos_1, pos_2 = bone_global_locations(rig, 'heelPosition_R')
pos_1[0] -= 0.03
pos_2[0] -= 0.03
new_bone_creation(pos_1, pos_2, 'MCH_footRoll_3_R', rig, 'footIK_R', False, 0)
pos_1, pos_2 = bone_global_locations(rig, 'heelPosition_R')
pos_1[0] += 0.03
pos_2[0] += 0.03
new_bone_creation(pos_1, pos_2, 'MCH_footRoll_2_R', rig, 'MCH_footRoll_3_R', False, 0)

pos_1, pos_2 = bone_global_locations(rig, 'heelPosition_L')
pos_1[0] += 0.03
pos_2[0] += 0.03
new_bone_creation(pos_1, pos_2, 'MCH_footRoll_3_L', rig, 'footIK_L', False, 0)
pos_1, pos_2 = bone_global_locations(rig, 'heelPosition_L')
pos_1[0] -= 0.03
pos_2[0] -= 0.03
new_bone_creation(pos_1, pos_2, 'MCH_footRoll_2_L', rig, 'MCH_footRoll_3_L', False, 0)
parent_bone(rig, 'heelPosition_L', 'MCH_footRoll_2_L', False)
parent_bone(rig, 'heelPosition_R', 'MCH_footRoll_2_R', False)

copy_rotation(rig, 'MCH_footRoll_2_L',subtarget='footRollCTRL_L', use_y=True, target_space='LOCAL', owner_space='LOCAL')
copy_rotation(rig, 'MCH_footRoll_2_R',subtarget='footRollCTRL_R', use_y=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_2_L', min_y=degree_to_radians(-170), max_y=0, limit_y=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_2_R', min_y=degree_to_radians(-170), max_y=0, limit_y=True, target_space='LOCAL', owner_space='LOCAL')

copy_rotation(rig, 'MCH_footRoll_3_L',subtarget='footRollCTRL_L', use_y=True, target_space='LOCAL', owner_space='LOCAL')
copy_rotation(rig, 'MCH_footRoll_3_R',subtarget='footRollCTRL_R', use_y=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_3_L', max_y=degree_to_radians(170), limit_y=True, target_space='LOCAL', owner_space='LOCAL')
limit_rotation(rig, 'MCH_footRoll_3_R', max_y=degree_to_radians(170), limit_y=True, target_space='LOCAL', owner_space='LOCAL')

# 28-33
pos_1, pos_2 = bone_global_locations(rig, 'MCH_footRoll_1_L')
new_bone_creation(pos_1, pos_2, 'heelHeight_L', rig, 'heelPosition_L', False, 0)
remove_constraints(rig, 'heelHeight_L')
length = bone_length('heelHeight_L', rig)
change_bone_length('heelHeight_L', rig, (length*2.5))
parent_bone(rig, 'heelHeight_L', 'heelPosition_L', False)
eular_change(rig, 'heelHeight_L', 'XYZ')
lock_rotation(rig, 'heelHeight_L', y=True, z=True)
lock_location(rig, 'heelPosition_L',x=True, z=True)
lock_rotation(rig, 'heelPosition_L', x=True, y=True, z=True, w=True)
lock_scale(rig, 'heelPosition_L', x=True, y=True, z=True)

pos_1, pos_2 = bone_global_locations(rig, 'MCH_footRoll_1_R')
new_bone_creation(pos_1, pos_2, 'heelHeight_R', rig, 'heelPosition_R', False, 0)
remove_constraints(rig, 'heelHeight_R')
length = bone_length('heelHeight_R', rig)
change_bone_length('heelHeight_R', rig, (length*2.5))
parent_bone(rig, 'heelHeight_R', 'heelPosition_R', False)
eular_change(rig, 'heelHeight_R', 'XYZ')
lock_rotation(rig, 'heelHeight_R', y=True, z=True)
lock_location(rig, 'heelPosition_R',x=True, z=True)
lock_rotation(rig, 'heelPosition_R', x=True, y=True, z=True, w=True)
lock_scale(rig, 'heelPosition_R', x=True, y=True, z=True)

# 35-38
transformation(rig, 'heelHeight_L', from_max_y=1, to_max_y=-1, map_from='LOCATION',map_to_y_from="Y", subtarget='heelPosition_L', target=rig,use_motion_extrapolate=True, owner_space="LOCAL", target_space='LOCAL')
copy_location(rig, 'toesIK_L', subtarget='heelHeight_L')
copy_rotation(rig, 'toes_L', subtarget='toesIK_L')
copy_rotation(rig, 'footFK_L', subtarget='MCH_foot_L')

transformation(rig, 'heelHeight_R', from_max_y=1, to_max_y=-1, map_from='LOCATION',map_to_y_from="Y", subtarget='heelPosition_R', target=rig,use_motion_extrapolate=True, owner_space="LOCAL", target_space='LOCAL')
copy_location(rig, 'toesIK_R', subtarget='heelHeight_R')
copy_rotation(rig, 'toes_R', subtarget='toesIK_R')
copy_rotation(rig, 'footFK_R', subtarget='MCH_foot_R')
...
# 39-43 -
start_point, global_pos, end_point, global_alt = object_location('knee_Empty_L', y=-.03, alter=True)
new_bone_creation(start_point, end_point, 'kneePole_L', rig, 'footIK_L', False, 0)
change_bone_length('kneePole_L', rig, .12)
parent_bone(rig, 'kneePole_L', 'footIK_L', False)
pos_1, pos_2 = bone_global_locations(rig, 'kneePole_L')
pos_1[1] += -.2
pos_2[1] += -.2
move_head_or_tail('kneePole_L', rig, True, True, pos_1, pos_2)

start_point, global_pos, end_point, global_alt = object_location('knee_Empty_R', y=-.03, alter=True)
new_bone_creation(start_point, end_point, 'kneePole_R', rig, 'footIK_R', False, 0)
change_bone_length('kneePole_R', rig, .12)
parent_bone(rig, 'kneePole_R', 'footIK_R', False)
pos_1, pos_2 = bone_global_locations(rig, 'kneePole_R')
pos_1[1] += -.2
pos_2[1] += -.2
move_head_or_tail('kneePole_R', rig, True, True, pos_1, pos_2)


# 44

### new ik location
create_IK(rig, 'tibia_L', 2, 0, 'kneePole_L', rig, 'MCH_foot_L', rig)
create_IK(rig, 'tibia_R', 2, degree_to_radians(-180), 'kneePole_R', rig, 'MCH_foot_R', rig)
